  ...
  const loginSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/login', {
        csw_username: loginUsername,
        csw_password: loginPassword
      })
  
      if (response.status === 200) {
        localStorage.setItem('token', response.data.token)
  
        const userPosition = response.data.user.csw_position
        
        if (userPosition === 'Manager' || userPosition === 'Employee') {
          navigateTo('/staff')
        } else {
          navigateTo('/')
        }
